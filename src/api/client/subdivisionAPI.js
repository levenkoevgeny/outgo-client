...
export const subdivisionAPI = {
  async getItemsList(token, searchForm = { user: "" }) {
    let { user } = searchForm
    return axios.get(
      `${process.env.VUE_APP_BACKEND_PROTOCOL}://${process.env.VUE_APP_BACKEND_HOST}:${process.env.VUE_APP_BACKEND_PORT}/api/${base_url}/?user=${user}`,
      authHeaders(token),
    )
  },
